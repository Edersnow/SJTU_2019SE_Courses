...
int main()
{
    ...
    const uint64_t valueSize1 = 1500;
    ...
    start1 = std::chrono::steady_clock::now();
    for (uint64_t i = 0; i < cycleNum; i += 2) {
        kvStore.del(i);
    }
    end1 = std::chrono::steady_clock::now();
    std::cout << "Total time for " + std::to_string(cycleNum / 2) + " DELs: "
              << std::chrono::duration_cast <std::chrono::milliseconds> (end1 - start1) .count()
              << "ms."
              << std::endl;
#endif
    ...
    const uint64_t valueSize1_ = 1024 * 10;
    ...
    const uint64_t valueSize2 = 1500;
