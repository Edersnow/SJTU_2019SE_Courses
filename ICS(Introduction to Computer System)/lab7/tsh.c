...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        // create child process and try exec commands
        pid = fork();
        if (pid < 0) {
        ...
        if (!bg) {
            ...
            printf("[%d] (%d) %s", tmp->jid, pid, tmp->cmdline);
        }
    }
}
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        // a job terminated by signal 
        if (WIFSIGNALED(status)){
            printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
            deletejob(jobs, pid);

        // a job stopped by signal
...
void sigint_handler(int sig) 
{
    // get fgpid and send sigint signal
    int pid = fgpid(jobs);
...
void sigtstp_handler(int sig) 
{
    // get fgpid and send sigtstp signal
    int pid = fgpid(jobs);
