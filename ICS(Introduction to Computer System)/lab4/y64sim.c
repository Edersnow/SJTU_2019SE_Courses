...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    case A_ADD:
        ...
        argB >>= 63;
        val >>= 63;
        if (argA == argB && argB != val)  ovf = TRUE;
        break;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    case I_RRMOVQ:  /* 2:x regA:regB */
        if (cond_doit(sim->cc, ifun))  set_reg_val(sim->r, rB, get_reg_val(sim->r, rA));
        sim->pc = next_pc;
        break;
    ...
    case I_RMMOVQ: /* 4:0 regA:regB imm */
        set_long_val(sim->m, imm + get_reg_val(sim->r, rB), get_reg_val(sim->r, rA));
        sim->pc = next_pc;
        break;

    case I_MRMOVQ: /* 5:0 regB:regA imm */
        if (!get_long_val(sim->m, imm + get_reg_val(sim->r, rB), &tmp)){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, imm + get_reg_val(sim->r, GET_REGB(codefun)));
            return STAT_ADR;
        }
    ...
    case I_ALU: /* 6:x regA:regB */
        tmp = compute_alu(ifun, get_reg_val(sim->r, rA), get_reg_val(sim->r, rB));
        set_reg_val(sim->r, rB, tmp);
        sim->cc = compute_cc(ifun, rA, rB, tmp);
        sim->pc = next_pc;
        break;
